...
@Component({
  selector: 'app-dispatchprint',
  templateUrl: './dispatchprint.component.html',
  styleUrls: ['./dispatchprint.component.scss']
})
export class DispatchprintComponent implements OnInit {
  ...
  constructor(public ds : DataServiceService,public route : ActivatedRoute) {
    route.params.subscribe(params => {
      if(params.id){
        this.ds.getData('/PrintDispatch/'+params.id).then((data:any) => {
          // console.log(data);
          this.basicdatails=data.recordset[0];
          this.details=data.recordsets[1];
          this.basicdatails.totalcount = this.verticalSum(this.details, "count1");
        },err => {
          console.log(err);
        })
      }else{
